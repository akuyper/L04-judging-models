...
Split the abalone data into a training set and a testing set. Use stratified sampling. You should decide on an appropriate percentage for splitting the data. This should be done in the `exercise_1/1_initial_setup.R` script, but remember to provide display code for graders.  
...
No display code is required for this task. Only need a confirmation that it has been completed. The output in the next task will verify that this has been done correctly. 
